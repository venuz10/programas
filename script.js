...
// Importar la librería jsPDF (asegúrate de incluirla en tu proyecto)
...
function rentarPelicula(categoria, index) {
  ...
  document.getElementById('rentaForm').onsubmit = function (e) {
    ...
    const doc = new jsPDF();
    doc.setFontSize(16);
    doc.text("Confirmación de Renta de Película", 20, 20);
    doc.setFontSize(12);
    doc.text(`Nombre: ${nombre}`, 20, 40);
    doc.text(`Correo: ${email}`, 20, 50);
    doc.text(`Película Rentada: ${pelicula.titulo}`, 20, 60);
    doc.text(`Precio: $15`, 20, 70);
    
    const pdfData = doc.output("blob");

    // Enviar el correo con EmailJS
    const formData = new FormData();
    formData.append("to", "vav23413070030477@cecyteh.edu.mx");
    formData.append("subject", "Confirmación de Renta");
    formData.append("message", `Hola ${nombre}, aquí está tu confirmación de renta.`);
    ...
    .then(() => {
      alert(`Gracias, ${nombre}. Se ha enviado la confirmación de renta a tu correo.`);
    })
    .catch(() => {
